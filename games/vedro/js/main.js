...
// Тысяча человечков — тысяча спрайтов в каждом кадре. На десктопе это ~2 мс,
// на телефоне заметно дороже, поэтому там толпа мельче (и порог падения тоже).
if (window.matchMedia && window.matchMedia('(pointer: coarse)').matches) {
  CONF.crowd.rows = 13;
  CONF.crowd.needed = 650;
}
...
const audio = $('track');
audio.volume = 0.5;
let musicBroken = false;              // трек не доехал: нет файла или нет сети
...
// Если играть нечем — гасим кнопку, чтобы она не обещала того, чего нет
audio.addEventListener('error', () => {
  musicBroken = true;
  renderMute();
  console.warn('[vedro] трек не загрузился:', audio.currentSrc || audio.src);
});

function renderMute() {
  muteMusicBtn.classList.toggle('off', musicMuted || musicBroken);
  muteMusicBtn.setAttribute('aria-label',
    musicBroken ? 'Музыка недоступна' : (musicMuted ? 'Включить музыку' : 'Выключить музыку'));
...
document.addEventListener('visibilitychange', () => {
  ...
  else if (!musicMuted && !musicBroken && window.__vedroReady) audio.play().catch(() => {});
});
...
const ENDINGS = {
  ...
  ascend: {
    head: CONF.loveLine,                            // ровно то же, что и в самой игре

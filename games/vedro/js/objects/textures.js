...
import { CONF, PAL, SHIRTS, HAIRS } from '../config.js';
...
const PERSON_FRAMES = {
  ...
  // руки к небу и ноги врозь: поза для финала, когда ведро уходит вверх
  cheer: (p, shirt, hair) => {
    p.r(0, 4, 2, 3, PAL.pack);
    p.r(2, 0, 3, 2, hair);
    p.r(2, 2, 3, 2, PAL.skin);
    p.r(4, 2, 1, 1, PAL.ink);
    p.r(0, 1, 1, 3, PAL.skin);                     // обе руки вверх
    p.r(6, 1, 1, 3, PAL.skin);
    p.r(1, 4, 5, 2, shirt);
    p.r(2, 6, 3, 2, shirt);
    p.r(2, 8, 3, 2, PAL.pants);
    p.r(1, 10, 1, 1, PAL.pants); p.r(5, 10, 1, 1, PAL.pants);
...
const HERO_FRAMES = {
  ...
  cheer: (p) => {
    p.r(1, 2, 1, 4, PAL.hair);                     // дреды
    p.r(2, 0, 4, 2, PAL.hair);
    p.r(2, 2, 3, 2, PAL.skin);
    p.r(4, 2, 1, 1, PAL.ink);
    p.r(0, 1, 1, 3, PAL.skin);                     // обе руки вверх
    p.r(7, 1, 1, 3, PAL.skin);
    p.r(1, 4, 6, 4, PAL.shoe);
    p.r(1, 6, 6, 1, PAL.pack);
    p.r(1, 5, 6, 1, PAL.rustHi);
    p.r(2, 8, 3, 2, PAL.pants);
...
// ─── Столб света с небес: узкий вверху, разливается книзу ───────────────────
// Рисуем построчно: каждая строка — горизонтальная полоса своей ширины
// и прозрачности. Так столб получается мягким по краям без размытия.
function makeBeam(scene) {
  if (scene.textures.exists('beam')) return;
  const W = 192, H = 320;
  const tex = scene.textures.createCanvas('beam', W, H);
  const ctx = tex.getContext();
  for (let y = 0; y < H; y++) {
    const k = y / H;                                // 0 вверху, 1 внизу
    const halfW = (8 + k * 84);
    const a = 0.40 * (1 - k * 0.55) * Math.min(1, k * 6);
    const g = ctx.createLinearGradient(W / 2 - halfW, 0, W / 2 + halfW, 0);
    g.addColorStop(0, 'rgba(255,230,150,0)');
    g.addColorStop(0.5, `rgba(255,248,214,${a.toFixed(3)})`);
    g.addColorStop(1, 'rgba(255,230,150,0)');
    ctx.fillStyle = g;
    ctx.fillRect(W / 2 - halfW, y, halfW * 2, 1);
  }
  tex.refresh();
}

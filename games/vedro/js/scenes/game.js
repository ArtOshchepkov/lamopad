// ─── Основная сцена: плита, пропасть, ведро и растущая толпа ────────────────
import { CONF, PAL } from '../config.js';
import { Debug } from '../debug.js';
import { Sfx } from '../sfx.js';
import { Bucket } from '../objects/bucket.js';
import { Crowd } from '../objects/crowd.js';
import { Player } from '../objects/player.js';

const hex = (c) => Phaser.Display.Color.HexStringToColor(c).color;

export class GameScene extends Phaser.Scene {
  constructor() { super('game'); }

  create() {
    const W = this.scale.width, H = this.scale.height;
    this.groundY = H - CONF.groundH;
    this.touch = this.sys.game.device.input.touch;

    this._buildWorld(W, H);

    this.bucket = new Bucket(this, this.groundY);
    this.player = new Player(this, this.groundY, this.bucket.footLeft);
    this.crowd = new Crowd(this, this.groundY, this.bucket.footLeft);

    // состояние забега
    this.state = 'play';                            // play | win
    this.ending = null;                             // 'topple' | 'ascend'
    this.elapsed = 0;
    this.left = CONF.timeLimit;                     // сколько секунд осталось
    this.started = false;
    this.peak = 1;
    this.hint = CONF.hints.move;
    this.soloSeen = false;
    this.dustT = 0;
    // scene.restart() переиспользует сам объект сцены, но убивает её детей:
    // всё, что живёт между забегами, обнуляем руками
    this.reported = false;
    this.glow = this.rays = this.rays2 = this.dim = this.beam = this.loveText = null;
    this.sparkT = 0;
    this.heroParty = false;
    this.partyT = 0;

    this._bindInput();
  }

  // ── мир: небо полосами, солнце, облака, плита и чернота за краем ──────────
  _buildWorld(W, H) {
    const horizon = this.groundY;
    const g = this.add.graphics().setDepth(0);
    const bands = [
      [PAL.skyTop, 0.00], [PAL.skyMid, 0.42], [PAL.skyLow, 0.78],
    ];
    for (let i = 0; i < bands.length; i++) {
      const y0 = Math.round(horizon * bands[i][1]);
      const y1 = i + 1 < bands.length ? Math.round(horizon * bands[i + 1][1]) : horizon;
      g.fillStyle(hex(bands[i][0]), 1);
      g.fillRect(0, y0, W, y1 - y0);
    }
    // дизеринг на стыках полос — честный NES-градиент
    for (let i = 1; i < bands.length; i++) {
      const y = Math.round(horizon * bands[i][1]);
      g.fillStyle(hex(bands[i - 1][0]), 1);
      for (let row = 0; row < 3; row++) {
        for (let x = (row % 2) * 8; x < W; x += 16) g.fillRect(x, y + row * 4, 8, 4);
      }
    }

    // солнце и облака — долями от высоты неба: в портрете его сильно больше
    this.add.image(0.33 * W, 0.2 * horizon, 'sun').setScale(5).setDepth(1).setAlpha(0.95);
    this.clouds = [];
    for (const [fx, fy, s] of [[0.27, 0.12, 4], [0.58, 0.26, 3], [0.86, 0.09, 5],
                               [0.44, 0.40, 2], [0.14, 0.55, 3], [0.72, 0.62, 2]]) {
      const c = this.add.image(fx * W, fy * horizon, 'cloud').setScale(s).setDepth(1).setAlpha(0.9);
      this.clouds.push({ img: c, speed: 4 + s * 2 });
    }

    // дальние холмы у горизонта — та сторона пропасти, до которой не дойти
    const hills = this.add.graphics().setDepth(1);
    for (let x = 0; x < W; x += 8) {
      const top = horizon - 34 - Math.abs(Math.sin(x * 0.011)) * 30 - Math.sin(x * 0.027) * 12;
      hills.fillStyle(hex(PAL.haze), 1);
      hills.fillRect(x, top, 8, horizon - top);
      hills.fillStyle(hex(PAL.hazeDark), 1);
      hills.fillRect(x, top, 8, 4);
    }

    // пропасть: за краем плиты — только темнота
    const ab = this.add.graphics().setDepth(2);
    ab.fillStyle(hex(PAL.abyss), 1);
    ab.fillRect(CONF.cliffX, horizon, W - CONF.cliffX, H - horizon);
    ab.fillStyle(hex('#181624'), 1);
    ab.fillRect(CONF.cliffX, horizon, W - CONF.cliffX, 10);
    // дальний берег — чтобы пропасть читалась как пропасть
    ab.fillStyle(hex(PAL.groundDark), 1);
    ab.fillRect(W - 34, horizon + 26, 34, H - horizon - 26);
    ab.fillStyle(hex(PAL.groundDim), 1);
    ab.fillRect(W - 34, horizon + 26, 34, 6);

    this.add.image(0, horizon, 'ground').setOrigin(0, 0).setDepth(3);
  }

  // ── ввод: клавиши, половинки экрана для тача, чит-коды ────────────────────
  _bindInput() {
    this.keys = this.input.keyboard.addKeys({
      left: Phaser.Input.Keyboard.KeyCodes.LEFT,
      right: Phaser.Input.Keyboard.KeyCodes.RIGHT,
      a: Phaser.Input.Keyboard.KeyCodes.A,
      d: Phaser.Input.Keyboard.KeyCodes.D,
    });

    this.touchDir = 0;
    const fromPointer = (p) => (p.x < this.scale.width / 2 ? -1 : 1);
    this.input.on('pointerdown', (p) => { this.touchDir = fromPointer(p); });
    this.input.on('pointermove', (p) => { if (p.isDown) this.touchDir = fromPointer(p); });
    this.input.on('pointerup', () => { this.touchDir = 0; });
    this.input.on('gameout', () => { this.touchDir = 0; });

    // чит-коды: копим последние нажатия по e.code — не зависит от раскладки
    this.buf = '';
    this.input.keyboard.on('keydown', (e) => {
      const m = /^Key([A-Z])$/.exec(e.code);
      if (!m) return;
      this.buf = (this.buf + m[1].toLowerCase()).slice(-8);
      if (this.buf.endsWith('lama')) { this.crowd.add(true); this.buf = ''; }
      else if (this.buf.endsWith('vedro')) { for (let i = 0; i < 40; i++) this.crowd.add(); this.buf = ''; }
      else if (this.buf.endsWith('padai')) { this._win(); this.buf = ''; }
      else if (this.buf.endsWith('dead')) { this._ascend(); this.buf = ''; }
    });
  }

  _dir() {
    if (!window.__vedroReady || this.state !== 'play') return 0;
    const k = this.keys;
    if (k.left.isDown || k.a.isDown) return -1;
    if (k.right.isDown || k.d.isDown) return 1;
    return this.touchDir;
  }

  update(time, delta) {
    const dt = Math.min(delta / 1000, 0.05);        // защита от возврата на вкладку

    for (const c of this.clouds) {                  // облакам всё равно
      c.img.x -= c.speed * dt;
      if (c.img.x < -60) c.img.x = this.scale.width + 60;
    }

    if (this.state === 'win') {
      this.bucket.update(dt, 0);
      this.crowd.party(dt);
      this._heroParty(dt);
      if (this.glow) this._shine(dt);
      if (this.bucket.gone && !this.reported) {
        this.reported = true;                       // дать полюбоваться финалом
        this.time.delayedCall(CONF.hold[this.ending] || CONF.hold.topple, () => this._report());
      }
      return;
    }

    const dir = this._dir();
    if (dir !== 0) this.started = true;
    if (this.started) {
      this.elapsed += dt;
      this.left = Math.max(0, CONF.timeLimit - this.elapsed);
      if (this.left === 0) this._ascend();
    }

    const pushing = this.player.update(dt, dir);
    this.crowd.update(dt, pushing);
    const force = pushing ? 1 + this.crowd.force : 0;
    this.peak = Math.max(this.peak, this.crowd.size + 1);

    this.bucket.update(dt, force);

    // крошка из-под точки опоры, пока ведро ворочается
    if (this.bucket.progress > 0.15) {
      this.dustT += dt;
      if (this.dustT > (0.22 - this.bucket.progress * 0.16) * (this.touch ? 2 : 1)) {
        this.dustT = 0;
        this.bucket.dust();
      }
    }

    if (force >= CONF.crowd.needed) this._win();

    this._hint(pushing, force);
    Debug.set('состояние', this.state);
    Debug.set('толпа', this.crowd.size);
    Debug.set('сила', force);
    Debug.set('наклон', this.bucket.tilt.toFixed(1));
    Debug.set('напряжение', (this.bucket.progress * 100).toFixed(0) + '%');
    Debug.set('время', this.elapsed.toFixed(1));
    Debug.set('осталось', this.left.toFixed(1));
  }

  _hint(pushing, force) {
    const C = CONF.hints;
    if (this.left <= 8 && this.started) this.hint = C.hurry;
    else if (this.crowd.size >= 30) this.hint = C.horde;
    else if (pushing && this.crowd.size > 0) this.hint = C.keep;
    else if (pushing) { this.hint = C.solo; this.soloSeen = true; }
    else if (this.player.x >= this.bucket.footLeft - 80) this.hint = C.push;
    else this.hint = this.soloSeen ? C.keep : C.move;
  }

  /** Финал первый: навалились и уронили. */
  _win() {
    if (this._finish('topple')) this.bucket.topple();
  }

  /** Финал второй: время вышло, ведро вознеслось. */
  _ascend() {
    if (!this._finish('ascend')) return;
    this.bucket.ascend();
    this._lightUp();
  }

  _finish(ending) {
    if (this.state === 'win') return false;
    this.state = 'win';
    this.ending = ending;
    this.peak = Math.max(this.peak, this.crowd.size + 1);
    this.crowd.freeze();
    this.player.person.setFrame('stand');
    this.time.delayedCall(500, () => {
      this.crowd.celebrate();
      this.player.person.setFrame('cheer');
      this.heroParty = true;
      Sfx.cheer();
    });
    return true;
  }

  /** Герой прыгает вместе со всеми: он тоже всё это затеял. */
  _heroParty(dt) {
    if (!this.heroParty) return;
    this.partyT = (this.partyT || 0) + dt;
    this.player.person.y = this.groundY
      - Math.abs(Math.sin(this.partyT * 5)) * CONF.crowd.partyAmp;
  }

  /** Свет великой мудрости: вспышка, столб с небес, лучи, ядро и искры. */
  _lightUp() {
    const W = this.scale.width, H = this.scale.height;
    const b = this.bucket.sprite;
    const cx = b.x - this.bucket.width * 0.45;
    const cy = b.y - b.displayHeight * 0.45;

    // мир вокруг гаснет — светится только ведро
    this.dim = this.add.rectangle(0, 0, W, H, 0x0a0820)
      .setOrigin(0, 0).setDepth(9).setAlpha(0);      // гаснет пейзаж, но не толпа
    this.tweens.add({ targets: this.dim, alpha: 0.55, duration: 1600 });

    // столб света: приходит сверху ещё до того, как ведро оторвалось
    this.beam = this.add.image(cx, 0, 'beam')
      .setOrigin(0.5, 0).setDepth(13).setBlendMode(Phaser.BlendModes.ADD)  // за толпой
      .setAlpha(0).setDisplaySize(340, this.groundY + 40);
    this.tweens.add({ targets: this.beam, alpha: 0.9, duration: 900, ease: 'Quad.easeOut' });

    // два слоя лучей навстречу друг другу — свет должен жить, а не висеть
    // свет — строго ЗА ведром (у ведра depth 15): иначе оно растворяется в нём
    this.rays = this.add.image(cx, cy, 'rays')
      .setDepth(14).setBlendMode(Phaser.BlendModes.ADD).setScale(0.2).setAlpha(0);
    this.rays2 = this.add.image(cx, cy, 'rays')
      .setDepth(14).setBlendMode(Phaser.BlendModes.ADD).setScale(0.2).setAlpha(0)
      .setRotation(0.22);
    this.glow = this.add.image(cx, cy, 'glow')
      .setDepth(14.5).setBlendMode(Phaser.BlendModes.ADD).setScale(1).setAlpha(0);
    this.tweens.add({ targets: this.rays, scale: 7, alpha: 0.85, duration: 2000, ease: 'Quad.easeOut' });
    this.tweens.add({ targets: this.rays2, scale: 5.6, alpha: 0.55, duration: 2400, ease: 'Quad.easeOut' });
    this.tweens.add({ targets: this.glow, scale: 6, alpha: 1, duration: 1500, ease: 'Quad.easeOut' });

    // вспышка ровно в момент отрыва
    this.time.delayedCall(700, () => {
      const flash = this.add.rectangle(0, 0, W, H, 0xfff6de)
        .setOrigin(0, 0).setDepth(19).setAlpha(0);
      this.tweens.add({
        targets: flash, alpha: 0.85, duration: 160, yoyo: true, hold: 60,
        onComplete: () => flash.destroy(),
      });
      this.cameras.main.shake(400, 0.004);
    });

    this._loveLine();
  }

  /** «МЫ ПОЛЮБИЛИ ВЕДРО» — прямо над толпой, той же мыслью, что и на итогах. */
  _loveLine() {
    const W = this.scale.width;
    const t = this.add.text(0, 0, CONF.loveLine, {
      fontFamily: '"Courier New", ui-monospace, monospace',
      fontSize: '44px', color: PAL.paper, fontStyle: 'bold',
      stroke: PAL.rustDeep, strokeThickness: 8,
    }).setOrigin(0.5, 1).setDepth(20).setAlpha(0).setScale(0.4);
    t.setShadow(0, 0, PAL.rustHi, 18, false, true);

    // держим надпись над толпой, но не даём ей вылезти за края экрана
    t.x = Phaser.Math.Clamp(this.crowd.midX, t.width / 2 + 12, W - t.width / 2 - 12);
    t.y = Math.max(t.height + 12, this.groundY - 150);
    this.loveText = t;

    this.tweens.add({
      targets: t, alpha: 1, scale: 1, duration: 700, delay: 1500, ease: 'Back.easeOut',
    });
    this.tweens.add({
      targets: t, y: t.y - 10, duration: 1600, delay: 2200,
      yoyo: true, repeat: -1, ease: 'Sine.easeInOut',
    });
  }

  /** Сияние живёт своей жизнью: лучи крутятся, ядро дышит, всё едет за ведром. */
  _shine(dt) {
    const b = this.bucket.sprite;
    const cx = b.x - this.bucket.width * 0.45;
    const cy = b.y - b.displayHeight * 0.45;
    this.rays.setPosition(cx, cy);
    this.rays2.setPosition(cx, cy);
    this.glow.setPosition(cx, cy);
    this.beam.x = cx;
    this.rays.rotation += 0.25 * dt;
    this.rays2.rotation -= 0.4 * dt;
    this.glow.setAlpha(0.8 + Math.sin(this.time.now / 260) * 0.15);
    this.beam.setAlpha(0.75 + Math.sin(this.time.now / 700) * 0.15);

    this.sparkT += dt;
    if (this.sparkT > 0.07) { this.sparkT = 0; this._spark(cx, cy); }
  }

  /** Крупинки света вокруг ведра: поднимаются и тают. */
  _spark(cx, cy) {
    const s = this.add.image(
      cx + (Math.random() - 0.5) * 300,
      cy + (Math.random() - 0.4) * 260,
      'glow',
    ).setDepth(16).setBlendMode(Phaser.BlendModes.ADD)
      .setScale(0.1 + Math.random() * 0.14).setAlpha(0.9);
    Sfx.sparkle();
    this.tweens.add({
      targets: s,
      y: s.y - 90 - Math.random() * 120,
      alpha: 0,
      scale: 0.02,
      duration: 900 + Math.random() * 700,
      onComplete: () => s.destroy(),
    });
  }

  _report() {
    this.game.events.emit('vedro-win', {
      ending: this.ending,
      count: this.peak,
      seconds: Math.max(1, Math.round(this.elapsed)),
    });
  }
}

...
export const Sfx = {
  ...
  /** На пике толпа прибывает десятками в секунду: без этого WebAudio захлебнётся. */
  _throttle(key, ms) {
  ...
  join()  {
    if (!this._throttle('join', 70)) return;
    this.tone(520, 0.07, { vol: 0.16 });
    this.tone(780, 0.07, { vol: 0.14, delay: 0.06 });
  },
  grunt() {
    if (!this._throttle('grunt', 70)) return;
    this.noise(0.13, { vol: 0.16, freq: 380, q: 4, sweepTo: 180 });
  },
  ...
  /** Вознесение: хор великой мудрости — аккорд, расходящийся вверх и ввысь. */
  ascend() {
    this.noise(3.4, { vol: 0.15, freq: 400, q: 0.4, sweepTo: 4200 });
    [262, 330, 392, 523, 659, 784, 1047].forEach((f, i) => {
      this.tone(f, 3.6 - i * 0.2, { type: 'triangle', vol: 0.13, delay: i * 0.2 });
      this.tone(f * 2, 2.4, { type: 'sine', vol: 0.07, delay: 0.6 + i * 0.2 });
    });
    // второй заход, когда ведро уже висит в свете: пафос должен нарастать
    [392, 523, 659, 784].forEach((f, i) =>
      this.tone(f, 3.2, { type: 'sine', vol: 0.09, delay: 2.6 + i * 0.12 }));
  },

  /** Ликование толпы + победная денди-фанфара. */
  cheer() {
    this.noise(2.0, { vol: 0.22, freq: 900, q: 0.5, sweepTo: 2200 });
    [523, 659, 784, 1047].forEach((f, i) =>
      this.tone(f, 0.16, { vol: 0.2, delay: 0.1 + i * 0.11 }));
    this.tone(1047, 0.5, { vol: 0.22, delay: 0.56 });
  },

  /** Искра: звонкая крупинка света, их много и они не должны давить. */
  sparkle() {
    if (!this._throttle('sparkle', 110)) return;
    const f = 1400 + Math.random() * 1600;
    this.tone(f, 0.12, { type: 'sine', vol: 0.05, to: f * 1.6 });
  },
};
